...
/**
 * Generate tasks based on survey results using AI
 ...
 * @param {string} familyId Family ID for getting family data
 * @returns {Promise<Array>} Generated tasks
 */
export async function generateTasks(weekNumber, previousTasks = [], surveyResponses = {}, familyId = null) {
  try {
    // Import AI service
    const aiService = await import('./aiService').then(module => module.default);
    
    // Get family data if familyId is provided
    let familyData = null;
    if (familyId) {
      try {
        familyData = await import('./DatabaseService').then(module => {
    ...
    // Create simple balance data
    const balanceData = {
      overallBalance: {
        mama: mamaPercentage,
        papa: papaPercentage
      },
      categoryBalance: categoryImbalances
    };
    
    // Generate tasks using AI service
    const tasks = await aiService.generateTaskRecommendations(
      familyData || { 
        familyName: 'Your Family',
        currentWeek: weekNumber,
        balance: balanceData.overallBalance 
      },
      balanceData,
      previousTasks,
      weekNumber
    ...
    return generateFallbackTasks(weekNumber, surveyResponses);
  }
}
...
function analyzeCategoryImbalances(surveyResponses) {
  ...
  Object.entries(surveyResponses).forEach(([key, value]) => {
    // Check if the key includes a category identifier
  ...
  Object.entries(categories).forEach(([category, counts]) => {
    if (counts.total > 0) {
      ...
      imbalances[category] = Math.abs(mamaPercent - papaPercent);
    } else {
...
// Fallback task generation function if AI fails
function generateFallbackTasks(weekNumber, surveyResponses) {
  // Analyze survey responses to determine imbalances
  const mamaResponses = Object.values(surveyResponses).filter(v => v === 'Mama').length;
  const papaResponses = Object.values(surveyResponses).filter(v => v === 'Papa').length;
  const totalResponses = mamaResponses + papaResponses;
  ...
  if (sortedCategories[2]) {
    const category = sortedCategories[2];
    const templates = taskTemplates[category];
